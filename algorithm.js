...
function calculateIdealProfile(inputs) {
  ...
  // ── BALANCE POINT ──────────────────────────────────────────────
  // Front-weighted: shorter forearm (less leverage), high throw angle, front grip
  // Rear-weighted: long forearm, low angle, rear grip
  // Middle: default
  let balance;
  const throwAngleDiff = (throwAngleDeg || naturalThrowAngle) - naturalThrowAngle;
  const isHighRelease = throwAngleDiff > 4;
  const isLowRelease  = throwAngleDiff < -4;
  const isLongForearm = leverageRatio > 0.153;
  const isShortForearm = leverageRatio < 0.143;

  if (throwingStyle === 'front' || isHighRelease || isShortForearm) {
    balance = 'front';
  } else if (throwingStyle === 'rear' || isLowRelease || isLongForearm) {
    balance = 'rear';
  } else {
    balance = 'middle';
  }
  ...
  if (throwingStyle === 'front') {
    barrelShape = idealWeight < 20 ? 'teardrop' : 'torpedo';
  } else if (throwingStyle === 'rear') {
    barrelShape = 'bomb';
  } else {
    if (idealWeight < 19) barrelShape = 'teardrop';
    else if (idealWeight < 23) barrelShape = 'torpedo';
    else if (idealWeight < 26) barrelShape = 'straight';
    else barrelShape = 'bomb';
  }
